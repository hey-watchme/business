...
def build_fact_extraction_prompt(
    ...
    Extracts raw facts/materials from transcription into the existing
    extraction_v1 structure without interpretation or evaluation.
...
個別支援計画書を作成するために、保護者ヒアリングから「支援の根拠となる素材・事実」を抽出します。
このフェーズの目的は、後段の分析に使うための材料をできるだけ漏れなく集めることです。
ここでは選別・評価・5領域への分類は行わず、まず具体的な情報を残すことを最優先してください。
...
以下のような内容は、捨てずにできるだけ具体的に残してください。

- 本人の行動・性格・特性
- 興味関心・好き嫌い・得意不得意
- 身体・感覚・運動・健康・生活習慣
- 家庭や園・学校などでの対人場面、集団場面
- 本人の発言、本人の気持ち、本人が言ったと保護者が伝えた内容
- 保護者の困りごと、不安、期待、希望
- スタッフの観察や会話の中で出た支援アイデア
- 事務連絡、雑談、周辺情報

重要:
- 事務連絡や雑談も、原則として無視せず `administrative_notes` に入れてください
- 一見些細でも、本人理解につながる内容は残してください
- どのカテゴリに入れるか迷う場合も削除せず、最も近いカテゴリに入れてください
...
✅ 好きな食べ物、遊び、困りごと、怪我、集中の様子など、些細に見える内容も残す
✅ 事務連絡や雑談も必要に応じて `administrative_notes` に残す
...
❌ 因果関係の推論（「〜のため」「〜が原因で」）
❌ 5領域に合わせるために事実を削ること
❌ 複数の具体的事実を1つの抽象表現にまとめすぎること

## 迷ったら残す原則
⚠️ **判断に迷う発言は、削除せずに抽出してください。**
- 要約しすぎず、可能な限り発言のニュアンスを維持した形で抽出すること
- どのカテゴリに入れるか迷う場合も削除せず、近いカテゴリに仮置きすること
- confidence を "low" に設定し、後段（Phase 2-3）に判断を委ねること
- 完全に支援文脈外の会話でも、無視せず administrative_notes に残してよい
...
{{
  "extraction_v1": {{
    ...
    "strengths": [
      {{
        "summary": "強み・得意なことの具体的内容",
        "confidence": "high/medium/low"
      }}
    ],
    "challenges": [
      {{
        "summary": "課題・困りごとの具体的内容",
        "confidence": "high/medium/low"
      }}
    ],
    "physical_sensory": [
      {{
        "summary": "身体・感覚に関する具体的内容",
        "confidence": "high/medium/low"
      }}
    ],
    "medical_development": [
      {{
        "summary": "医療・発達に関する具体的内容",
        "confidence": "high/medium/low"
      }}
    ],
    "family_environment": [
      {{
        "summary": "家族・環境に関する具体的内容",
        "confidence": "high/medium/low"
      }}
    ],
    "parent_intentions": [
      {{
        "summary": "保護者の希望・不安・困りごとの具体的内容",
        "priority": 1,
        "confidence": "high/medium/low"
      }}
    ],
