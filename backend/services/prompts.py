...
1. **5領域をバランスよくカバー**: 健康・生活、運動・感覚、認知・行動、言語・コミュニケーション、人間関係・社会性の全てについて support_items を生成すること
...
6. **情報不足時の対応**: ヒアリング内容から十分な情報が得られない項目は、無理に推測・一般論を入れず「情報が取得できませんでした」と記載する。特に以下に注意：
   - family_child_intentions.child/parents: 本人・保護者の発言がない場合は「情報が取得できませんでした」
   - support_items[].notes: 留意事項が不明な場合は「特記事項なし」または空文字列
   - transition_support.partner_organization: 連携先機関の情報がない場合は「情報が取得できませんでした」
7. **notes フィールドは必須**: 各 support_item に notes フィールドを必ず含め、専門的支援実施加算や配慮事項を記載する
