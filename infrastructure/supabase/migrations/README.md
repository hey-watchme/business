...
| ファイル | 状態 | 備考 |
|---------|------|------|
| `000_base_tables.sql` | ✅ 完了 | facilities, users, subjects テーブル作成 |
| `001a_alter_existing_tables.sql` | ✅ 完了 | users に role, facility_id 追加 |
| `002_cleanup_and_final_setup.sql` | ✅ 完了 | 古いテーブル削除、新規テーブル作成 |

**現在のデータベースは最新の状態です。追加のマイグレーション実行は不要です。**
...
-- facilities, users, subjects テーブルを作成
```

#### Step 2: 既存テーブルの拡張
```sql
-- 001a_alter_existing_tables.sql の内容を実行
-- public.usersテーブルにroleとfacility_idカラムを追加
```

#### Step 3: クリーンアップと最終設定
```sql
-- 002_cleanup_and_final_setup.sql の内容を実行
-- 古い business_children, business_facilities を削除
-- business_support_plans, subject_relations を作成
-- RLSポリシー、トリガー、ビューを設定
```
