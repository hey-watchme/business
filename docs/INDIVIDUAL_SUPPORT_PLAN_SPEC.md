...
**システム状態**: Phase 0-3パイプライン完全稼働 ✅、Phase 2アノテーション方式転換完了 ✅、自動sync実装完了 ✅、手動入力対応 ✅、複数LLMモデル選択対応 ✅、auto_chain制御実装完了 ✅
**実装完了度**: 99% (Phase 2アノテーション方式転換完了、プロンプト保存フロー修正完了、タブUI実装完了、パイプライン実行制御改善完了)
...
**ステータスは前進のみ**: 一度 `completed` になったセッションは、スポット再分析が失敗しても `completed` を維持する。スポット実行の失敗は `error_message` カラムのみに記録される。
...
| 4 | `analyzing` | 事実抽出中 / 事実整理中 / 個別支援計画生成中 | Phase 1〜3のいずれかを実行中（標準ルートのみ遷移） |
| 5 | `completed` | 個別支援計画書作成完了 | Phase 3完了＋計画書同期完了 |
| - | `error` / `failed` | エラー発生 | 標準ルートの途中フェーズで失敗（スポット実行では発生しない） |

### ステータス保護ルール（2026-02-26追加）

| 実行モード | 開始時 | 成功時 | 失敗時 |
|-----------|--------|--------|--------|
| **標準ルート**（Lambda / auto_chain=true） | `analyzing`に遷移 | `completed`に遷移 | `failed`に遷移 |
| **スポット実行**（auto_chain=false） | `completed`なら維持 | `completed`を維持 | `completed`を維持 + `error_message`のみ更新 |
| **Phase 2/3 単独実行** | statusを変更しない | statusを変更しない | statusを変更しない + `error_message`のみ更新 |
...
**タブ構造**（2026-02-26更新）:
0. **アセスメントタブ**: セッション情報、トランスクリプション、手動入力
1. **事実抽出タブ**: Phase 1結果（extraction_v1）、プロンプト編集・再実行
2. **事実整理タブ**: Phase 2結果（annotated_facts_v1）、プロンプト編集・再実行
3. **計画生成タブ**: Phase 3結果（assessment_v1）、プロンプト編集・再実行
4. **個別支援計画書タブ**: Excel風UI、2カラム版管理（AI生成 / ユーザー編集）
...
| 実行方法 | モデル選択 | auto_chain | 動作 |
|---------|----------|-----------|------|
| **全Phase一括実行** | モーダルで選択 | `false` | フロントエンドがPhase 1→2→3を個別に順次呼び出し |
| **Phase Xのみ再実行** | タブ内ドロップダウン | `false` | 該当Phaseのみ実行（ステータス保護あり） |
| **Lambda自動実行** | デフォルト（`gpt-4o`） | `true`（デフォルト） | Phase 1完了後、自動的にPhase 2→3を連鎖実行 |
...
| `use_custom_prompt` | bool | `false` | DBに保存済みのプロンプトを使用するか |
| `auto_chain` | bool | `true` | Phase 1完了後にPhase 2→3を自動連鎖するか |
| `provider` | string | 環境変数 | LLMプロバイダー（"openai", "gemini"） |
| `model` | string | 環境変数 | モデル名（"gpt-4o"等） |
| `custom_prompt` | string | null | プロンプトを直接渡す場合 |
